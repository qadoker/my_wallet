...
class AddOperation extends StatefulWidget {
  ...
  AddOperation(
      {this.newPrice, this.categoryVal, this.isAddedOperation, this.operation});
      ...
      body: SingleChildScrollView(
        child: Container(
          ...
          child: Column(children: [
                  ...
                  child: StreamBuilder<List<Categorie>>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.hasData) {
                          ...
                          if (_data.length == 0) {
                            Provider.of<WalletDatabase>(context)
                                .categoryDao
                                .insertCategory(Categorie(
                                    iconName: Icons.credit_card.codePoint,
                                    name: 'Maaş'));
                            Provider.of<WalletDatabase>(context)
                                .categoryDao
                                .insertCategory(Categorie(
                          ...
                          return DropdownButton(
                            ...
                            isExpanded: true,
                            value: widget.categoryVal != null
                                ? widget.categoryVal
                                : widget.categoryVal = snapshot.data[0],
                          );
            ...
            SizedBox(height: screenHeight * 0.05),
            TextButton(
                child: Text(widget.isAddedOperation == false ? appLang.edit : appLang.add,
                    style: TextStyle(color: blue, fontSize: 20.0)),
                onPressed: () {
                  setState(() {
                    print('${widget.categoryVal.name}, ${widget.newPrice}');
                    if (widget.newPrice != null &&
                        widget.categoryVal.name != null) {
                      final dao =
                          Provider.of<WalletDatabase>(context, listen: false)
                              .operationDao;
                      if (widget.isAddedOperation == false) {
                        if (double.parse(widget.newPrice) >= 0) {
                          dao.updateOperation(widget.operation.copyWith(
                              income: double.parse(widget.newPrice),
                              categoryName: widget.categoryVal?.name));
                        } else {
                          dao.updateOperation(widget.operation.copyWith(
                              expense: double.parse(widget.newPrice),
                              categoryName: widget.categoryVal?.name));
                        }
                      } else {
                        if (double.parse(widget.newPrice) >= 0) {
                          final operation = OperationsCompanion(
                              ...
                              categoryName:
                                  moor.Value(widget.categoryVal?.name));
                          dao.insertOperation(operation);
                        } else {
                          final operation = OperationsCompanion(
                              expense:
                                  moor.Value(double.parse(widget.newPrice)),
                              categoryName:
                                  moor.Value(widget.categoryVal?.name));
                          dao.insertOperation(operation);
                        }
